...
function loadhiststep(nx,ny,n) { const re = new RegExp('.*([0123456789]+).*',"g") ; let h = Module.pastehistBox.value; let m = [...h.matchAll(re)]; let moves = m.map((mm) => Number(mm[1])); m.map(mm => alert(mm[1])); if(n<moves.length){ return moves[n]; } else{ return -1; } }
...
var ___stop_em_js = Module['___stop_em_js'] = 67825;
