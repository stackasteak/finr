...
function loadhiststep(nx,ny,n) { const re = new RegExp("[^\d]*(\d+)[^\d]*","g") ; let h = Module.pastehistBox.value; let m = [...h.matchAll(re)]; let moves = m.map((mm) => Number(mm[1])); alert(moves.length); if(n<moves.length){ return moves[n]; } else{ return -1; } }
...
var ___stop_em_js = Module['___stop_em_js'] = 67817;
